...
public class AccountService : IAccountService
{
    ...
    // ------------------------------------------------------------------------------------------------------------------
    public async Task<AuthModel> RegisterInvestor(RegisterInvestor model)
    {
        ...
        {
            if (model.Img!=null)
                imgUrl = await _fileHandling.UploadFile(model.Img, "InvestorImg");
            else
        ...
        {
            var Investor = await _userManager.FindByNameAsync(model.Email);
            ...
            return new AuthModel
            {
                ...
                Roles = new List<string> { "Investor" },
                UserType = UserType.Investor,
                UserImgUrl = Investor.UserImgUrl,
                Job = Investor.Job,
            ...
            if (model.Img != null)
            {
                switch (user.UserType)
                {
                    ...
                    case UserType.Youth: imgUrl = await _fileHandling.UploadFile(model.Img, "YouthImg", user.UserImgUrl);
                        break;
                    case UserType.Investor: imgUrl = await _fileHandling.UploadFile(model.Img, "InvestorImg", user.UserImgUrl);
                        break;
                }
            }
        }
